...
const Footer = () => {
  ...
  return (
    <Flex w='100%' h='max-content' direction='column' as='footer'>
      ...
      <Flex w='100%' h={{base:HEIGHT_FOOTER_BASE, sm:HEIGHT_FOOTER_SM}} bgColor='bgColor' justifyContent='space-between' color='text' direction={{base:'column', sm:'row'}} py='15px' px={{base:PADDING_WEB_BASE, md:PADDING_WEB_SM}}>
        <Flex w={{base:'100%', sm: pathname === '/ubicacion' ? '100%' : '50%'}} direction='column' alignItems={{base:'center', sm:pathname === '/ubicacion' ? 'center' : 'start'}} justifyContent='center' fontWeight={300} fontSize={{base:'14px', sm:'18px'}} gap={2} borderRight={{base:'none', sm: pathname === '/ubicacion' ? 'none' : '1px solid #7A2D2C'}}>
          ...
          <Flex alignItems='center' gap={2} fontWeight={500}><Text>WhatsApp: </Text><Text color='highlight'>3516624922</Text></Flex>
